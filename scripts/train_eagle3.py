...
def parse_args() -> Tuple[ArgumentParser, Namespace]:
    ...
    training_group.add_argument("--save-strategy", type=str, default="steps", choices=["steps", "best"])
...
def sanity_check(args: Namespace) -> None:
    ...
    assert not(args.load_best_model_At_end and args.save_strategy != "best"), "--load-best-model-at-end requires --save-strategy to be 'best'"
    assert args.eval_interval % args.save_interval == 0, "--eval-interval should be a multiple of --save-interval to ensure proper best model tracking"
...
def save_checkpoints(
    ...
    epoch_output_dir = os.path.join(args.output_dir, f"{PREFIX_CHECKPOINT_DIR}_{epoch}_step_{step}")
    if is_best:
    ...
    with FSDP.state_dict_type(eagle3_model, StateDictType.FULL_STATE_DICT):
        ...
        if dist.get_rank() == 0:
            ...
            rotate_checkpoints(
                args.output_dir, args.save_total_limit, best_model_checkpoint, False, PREFIX_CHECKPOINT_DIR
            )
        dist.barrier()
...
def main():
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step_in_epoch, data in enumerate(progress_bar):
            ...
            if (
                ...
                idx = int(idx)
                current = (eval_acces[idx] if kind == "acc" else eval_plosses[idx]).item()
                is_best = (current > best_metric) if args.greater_is_better else (current < best_metric)
                if is_best:
                    best_metric = current
            ...
            if global_step % args.save_interval == 0:
                ...
                ckpt_path = save_checkpoints(
                    ...
                    is_best=current_is_best, best_model_checkpoint=best_model_checkpoint
                )
                if current_is_best:
    ...
    if dist.get_rank() == 0 and args.load_best_model_at_end and best_model_checkpoint is not None:
